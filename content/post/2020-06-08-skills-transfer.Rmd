...
A support concept describe the flow of activities and items. Normally we have at least two levels of support. Starting with basic maintenance which happens at the organization on the system. Intermediate maintenance is at facilities located close by with more skilled personnel and better equipped workshops. Anything beyond this goes to highly skilled personnel typically at manufacturers facilities.

Some organizations cannot afford multiple levels of support, and most support organizations face a shortage of some skills some time. When facing these situations the approach is often to transfer skills and grow the in-house capabilities. To do that we need to identify what is it that can be done in-house. 

A well defined task will identify the skills, tools, spares, facilities and information you need to be able to carry out the task. If all these resources already exist in-house then there is a close match to where we want to be. If none of the resources exist in-house then there will be a big gap to overcome.
...
a methodology that can assist in skills transfer. solution  can identify similar maintenance tasks in the system that are being executed at different support levels. The idea is that if two different tasks require similar skills and tools then a maintainer with the same skills and tools in another organisation can be taught how to do the additional tasks.  The heatmap below illustrate the similarity in the group (any number of groups can be used). The algorithm use unsupervised machine learning called, K-means clustering


The capability gap between maintenance levels determines the level of effort to assimilate some of the tasks done at another maintenance level. 
...
Can the resources the be used to identify the support level?
...
<!-- Clusterering of tasks does not replace a LORA, CORA, these are applied and most effectively during the early life stages of the system. What we focus on here is a way for the organisation to adapt to dynamic environment during the in-service phase of the equipment. Where skills sets are people already employed and that have the potential to grow in-house capabilities, and even the tools and spares could exist too.  -->

<!-- Ideas we must explore more is the fact that the dendrogram of resources, specifically parts show some parts are close in task relevancy, this is indicator to us that these parts are often used together, we can take this further and might find better efficiencies if we arrange our workshop so that these items are stored together, or handed out in kits together. Same with tool, if they not alrerady together in a toolbox. -->
